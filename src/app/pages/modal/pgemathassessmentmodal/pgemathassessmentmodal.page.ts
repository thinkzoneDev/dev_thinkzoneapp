...
@Component({
  selector: 'app-pgemathassessmentmodal',
  templateUrl: './pgemathassessmentmodal.page.html',
  styleUrls: ['./pgemathassessmentmodal.page.scss']
})
export class PgemathassessmentmodalPage {
  assessment_list: any = [];
  assessmenttest: any = [];
  ...
  async gettchassessmenttest() {
    const loading = await this.loadingController.create({});
      ...
      await this.api.gettchassessmenttest(this.studentid, this.program, this.class, this.stage, "math")
        .subscribe(res => {
          if(res.length > 0){
  ...
  async gettchassessment() {
    ...
    const loading = await this.loadingController.create({});
      await loading.present();
      await this.api.gettchassessment(preflanguage, this.program, this.class, this.stage, "math")
        .subscribe(res => {
