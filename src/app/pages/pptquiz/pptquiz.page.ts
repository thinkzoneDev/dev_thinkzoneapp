...
@Component({
  selector: "app-pptquiz",
  templateUrl: "./pptquiz.page.html",
  styleUrls: ["./pptquiz.page.scss"],
})
export class PPTQuizPage implements OnInit {
  // quiz;
  qno = 1;
  submitButton;
  submitButtonText;
  ...
  async finishQuiz() {
    ...
    this.quiz.forEach((element) => {
      ...
      this.user_quiz.push({
        user_answer: element.selectedOption,
      });
    });
        ...
        .subscribe(
          (res) => {
            if (res.status == "success") {
              ...
              this.showAlert("Info", "", "Something wrong happened");
            }
          },
          (err) => {
            this.serverDownMsg.presentToast();
