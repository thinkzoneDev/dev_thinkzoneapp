...
export class ManagerPage implements OnInit {
  ...
  // ----------- new code --------------
  async getfilelist() {
    await this.api.getallfilelistbyapptype("teacherapp").subscribe(
      (res) => {
        let value: any = [];
        if (res.length <= 0) {
          this.filelist = [];
          ...
          res.forEach((obj) => {
            if (
              obj.type == "file" &&
              (obj.s3directory == undefined ||
                obj.s3directory == null ||
                obj.s3directory == "")
            ) {
              value.push(obj);
            }
  ...
  async getfolderlist() {
    await this.api.getdistinctdirectorylistbyapptype("teacherapp").subscribe(
      (res) => {
        this.folderlist = res;
      },
      (err) => {}
