...
export class FileDisplayPage implements OnInit {
  ...
  async file_btn_click(file_obj, file_index){
    ...
    /* File obj is like: 
    {
      "path":
            {
              "displayname":"Introduction to Addition - Odia.mp4",
              "s3name":"1590067609083.mp4",
              "filetype":"mp4",
              "s3path":"https://shared001.s3.us-east-2.amazonaws.com/1590067609083.mp4"
            },
      "played":false
    }
    */
    ...
    this.file.checkDir(this.file.externalDataDirectory, local_directory).then(_ => 
      this.file.checkFile(this.file.externalDataDirectory, local_directory+'/'+ fileName).then(_ => {
        ...
        this.file.createDir(this.file.externalDataDirectory, local_directory, false).then(response => {
