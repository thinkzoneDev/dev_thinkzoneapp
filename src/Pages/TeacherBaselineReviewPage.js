...
const TeacherBaselineReviewPage = ({route, navigation}) => {
  // console.log('route--->', route.params.examType);
  const [examType, setExamType] = useState(route.params.examType);
  ...
  useEffect(() => {
    ...
    API.get(
      ...
      `getteacherbaselinedata/${examType}/od/${user[0].userid}/${user[0].usertype}`,
    )
      .then(res1 => {
        const datas = res1.data;
        // console.log('datas----->', datas[0].assesmentquestion);
        setReviewQuestions(datas[0].assesmentquestion);
  ...
  const postModuleidHeader = (title, index) => {
...
export default TeacherBaselineReviewPage;
