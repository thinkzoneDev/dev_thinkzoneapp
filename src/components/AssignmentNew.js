...
const AssignmentNew = ({navigation, route, question, uploadFile}) => {
  ...
  const fileUpload = () => {
    ...
    Api.post(`s3api/uploadroot/${s3name}`, formData, config).then(
      response => {
        ...
        if (response.data.s3path.length > 0) {
          Alert.alert(
            ...
            [
              {
                text: 'OK',
                onPress: () => navigation.navigate('monthlytrainingnew'),
                // onPress: () => null,
          ...
          uploadFile(response.data.s3path);
        }
      },
      err => {
        console.log(err);
